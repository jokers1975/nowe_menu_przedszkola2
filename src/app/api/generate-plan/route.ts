...
import { db } from "@/db";
import { allergens, globalDishAllergens, globalDishes, menuItems } from "@/db/schema";
...
export async function POST(req: Request) {
  try {
    ...
    // Prompt
    // ------------------------------------------------------------------
    const catalogText = MEAL_TYPES.map((mt) => {
      const list = byType[mt];
      if (list.length === 0) return `${mt}: (brak pozycji w katalogu)`;
...
- Dla każdej pary (data, typ_posilku) zwróć dokładnie JEDNO danie właściwego typu.
...
Następnie wypełnij pole "warnings" krótkimi uwagami dietetycznymi, jeśli widzisz coś niepokojącego.`;
    ...
    const output = object.items
      .map((it) => {
        ...
        if (!d) return null;
        if (d.type !== it.mealType) return null;
        return {
          date: it.date,
